...
##matrix to store results in
	...
	my.seq<-sort(c(seq(0,1.0,length.out=11),c(0.85,0.95,0.975,0.99)))
	
	param_lethality<-rep(my.seq,9)
	        
	param_sex<-c(rep(0.4,45),rep(0.9,90))
	param_sexresist<-c(rep(0.05,90),rep(0.85,45))
	param_selfish<-rep(rep(c(10,20,100),each=15),3)
...
##Cost of resistance = selection coefficient of resistant allele in wildtype environment without fungicide
	cost_resistance<-c(cost_resistance_A=0.05,cost_resistance_B=0.2,cost_resistance_C=0.2,cost_resistance_D=param_sexresist[param],cost_resistance_E=0.05,cost_resistance_F=0.05) 
	
	my.model<-8
	
##Gene drive parameters - TIM ADDED INOCULUM SIZE OF GENE DRIVE SPORES
		gene_drive.params<- c(Lethality = param_lethality[param] ,Selfishness = param_selfish[param], Fecundity_Inhibition = 0, Infection_Inhibition = 0, Growth_Inhibition = 0, Sex_Inhibition = 0, Survival_Inhibition=0, Inoculum = 50)
...
grand.res<-rbind(grand.res,res)
	...
	plot(param_lethality[subs],1-grand.res[subs,"Freq50_F"],ylab="Frequency drive allele",xlab="",type="l",las=1,cex.axis=cx,cex.lab=cx,lty=3)
	subs<-(param_sex==0.4)&(param_selfish==20)
	lines(param_lethality[subs],1-grand.res[subs,"Freq50_F"],type="l",lty=2)
	subs<-(param_sex==0.4)&(param_selfish==100)
	lines(param_lethality[subs],1-grand.res[subs,"Freq50_F"],type="l",lty=1)

	subs<-(param_sex==0.4)&(param_selfish==10)
	plot(param_lethality[subs],grand.res[subs,yield_type],ylim=c(0.5,1.0),ylab="% Yield",xlab="",type="l",las=1,cex.axis=cx,cex.lab=cx,lty=3)
	subs<-(param_sex==0.4)&(param_selfish==20)
	lines(param_lethality[subs],grand.res[subs,yield_type],type="l",lty=2)
	subs<-(param_sex==0.4)&(param_selfish==100)
	lines(param_lethality[subs],grand.res[subs,yield_type],type="l",lty=1)
	legend("left", bty="n",inset=.05,legend=c("100x","20x","10x"),lty=c(1:3),cex=cx)

	subs<-(param_sex==0.4)&(param_selfish==10)
	plot(param_lethality[subs],grand.res[subs,"Freq50_D"],ylim=c(0,1.0),ylab="Frequency sex allele",xlab="r (lethality)",type="l",las=1,cex.axis=cx,cex.lab=cx,lty=3)
	subs<-(param_sex==0.4)&(param_selfish==20)
	lines(param_lethality[subs],grand.res[subs,"Freq50_D"],type="l",lty=2)
	subs<-(param_sex==0.4)&(param_selfish==100)
	lines(param_lethality[subs],grand.res[subs,"Freq50_D"],type="l",lty=1)

	##Mainly sex
	
	subs<-(param_sex==0.9)&(param_selfish==10)&(param_sexresist==0.05)
	plot(param_lethality[subs],1-grand.res[subs,"Freq50_F"],ylab="",xlab="",type="l",las=1,cex.axis=cx,cex.lab=cx,lty=3)
	...
	subs<-(param_sex==0.9)&(param_selfish==10)&(param_sexresist==0.05)
	plot(param_lethality[subs],grand.res[subs,yield_type],ylim=c(0.5,1.0),ylab="",xlab="",type="l",las=1,cex.axis=cx,cex.lab=cx,lty=3)
	subs<-(param_sex==0.9)&(param_selfish==20)&(param_sexresist==0.05)
	lines(param_lethality[subs],grand.res[subs,yield_type],type="l",lty=2)
	subs<-(param_sex==0.9)&(param_selfish==100)&(param_sexresist==0.05)
	lines(param_lethality[subs],grand.res[subs,yield_type],type="l",lty=1)
	...
	subs<-(param_sex==0.9)&(param_selfish==10)&(param_sexresist==0.05)
	plot(param_lethality[subs],grand.res[subs,"Freq50_D"],ylim=c(0,1.0),ylab="",xlab="r (lethality)",type="l",las=1,cex.axis=cx,cex.lab=cx,lty=3)
	subs<-(param_sex==0.9)&(param_selfish==20)&(param_sexresist==0.05)
	lines(param_lethality[subs],grand.res[subs,"Freq50_D"],type="l",lty=2)
	subs<-(param_sex==0.9)&(param_selfish==100)&(param_sexresist==0.05)
	lines(param_lethality[subs],grand.res[subs,"Freq50_D"],type="l",lty=1)
...
write.csv(grand.res,file="DiploidGeneDriveFungicideResults.csv",row.names=F)