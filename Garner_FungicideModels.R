...
##matrix to store results in
	...
	param_costs<-rep(seq(0.0,0.2,length.out=5),4)
	param_model<-c(rep(2,5),rep(3,5),rep(4,5),rep(5,5))
...
##Wild-type (ABCDEF) parameter values for asexual stages
	...
	wt.params<-c(Fecundity=1.3,Infection=0.9,Growth=1.5, Pro_sex=0.4, Survival=0.9,Blank=1)
...
##Cost of resistance = selection coefficient of resistant allele in wildtype environment without fungicide
	cost_resistance<-c(cost_resistance_A=0.05,cost_resistance_B=0.05,cost_resistance_C=0.05,cost_resistance_D=0.05,cost_resistance_E=0.05,cost_resistance_F=0.05) 
	cost_resistance[]<-param_costs[param]
	
	my.model<-param_model[param]
...
##The set of possible models to consider
	model.choices<-c("No_control","One_Fungi_20","Two_Fungi_Simult","Two_Fungi_Alt","Two_Fungi_Season","HaploidGeneDrive","DiploidGeneDrive","HaploidFungicide","DiploidFungicide")
##change my.model to select a model
	...
	## d) Alternating fungicides every 5 years
	 if (which.model=="Two_Fungi_Alt") {
	  ...
	  if (which.model=="Two_Fungi_Season") {
	    which.fungicide<-c(2,3)
	    fungicide[20:50,1:2, which.fungicide[1]]<-2
	    fungicide[20:50,3:4, which.fungicide[2]]<-2
	  }
...
grand.res<-rbind(grand.res,res)
	...
	file.name4<-paste0("Figures/Model",my.model,"_params_yield_resistance",tmp,".pdf")
	
	pdf(file=file.name4,height=5.5,width=2.5)
	
	par(mfcol=c(3,1),mar=c(2,4,2,1),oma=c(2,0,0,0))
	
	plot(param_costs[1:5],grand.res[1:5,"mean_21_50"],ylab="Mean Yield",xlab="j (cost of resistance)",type="b",las=1,ylim=c(0.5,max(grand.res[,"mean_21_50"])),cex.axis=cx,cex.lab=cx)
	lines(param_costs[1:5],grand.res[6:10,"mean_21_50"],lty=2,type="b",pch=2)	
	lines(param_costs[1:5],grand.res[11:15,"mean_21_50"],lty=3,type="b",pch=3)	
	lines(param_costs[1:5],grand.res[16:20,"mean_21_50"],lty=4,type="b",pch=4)	
	
	plot(param_costs[1:5],1-(grand.res[1:5,c("Freq40_B")]),ylab="Freq resistance",xlab="j (cost of resistance)",type="b",las=1,ylim=c(0,1),cex.axis=cx,cex.lab=cx)
	lines(param_costs[1:5],1-rowMeans(grand.res[6:10,c("Freq40_B","Freq40_C")]),lty=2,type="b",pch=2)	
	lines(param_costs[1:5],1-rowMeans(grand.res[11:15,c("Freq40_B","Freq40_C")]),lty=3,type="b",pch=3)	
	lines(param_costs[1:5],1-rowMeans(grand.res[16:20,c("Freq40_B","Freq40_C")]),lty=4,type="b",pch=4)	
	legend(x=0,y=0.7,bty="n",ncol=1,legend=c("One","Simultaneous","Alt 5 years","Alt season"),lty=1:4,pch=1:4)
	dev.off()	
	
	#file.name3<-paste0("Scenario",scenario,".csv")

write.csv(grand.res,file="FungicideModelResults.csv",row.names=F)