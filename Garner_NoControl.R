...
legend("bottomright", bty="n",inset=.05, legend=c("Yield","Load"),lty=c(1,3),cex= cx)
...
legend("bottomright", bty="n",inset=.05,legend=c("Asex","Sex"),col=c('darkblue','red'),lty=1,cex= cx)
...
grand.res<-rbind(grand.res,res)
	...
	matplot(params,yield_params[,-(1:20)],type="p",ylab="% Yield",xlab="g",las=1,pch=20,cex=0.3,col="black",xlim=c(0,max(params)),cex.axis= cx,cex.lab= cx)	
	mtext(text = "g (Number infection cycles)",side=1,line=2,cex=0.7*cx)
	dev.off()
	
	
	#file.name3<-paste0("Scenario",scenario,".csv")

write.csv(grand.res,file="NoControlResults.csv",row.names=F)